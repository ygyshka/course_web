...
def send_mailing(mailing):
    ...
    recipient_list = [client.contact_mail for client in mailing.client_id]
...
def check_mailing():
    now = timezone.now()
    mailings = Mailing.objects.filter(status_code='create', time__lte=now.time(), next_send__lte=now.date())
